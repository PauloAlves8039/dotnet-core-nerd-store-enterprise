    ...
    public class ItemCarrinhoValidation : AbstractValidator<CarrinhoItem>
    {
        public ItemCarrinhoValidation()
        {
            ...
            RuleFor(c => c.Quantidade)
                .GreaterThan(0)
                .WithMessage(item => "A quantidade miníma para o é 1");

            RuleFor(c => c.Quantidade)
                .LessThan(CarrinhoCliente.MAX_QUANTIDADE_ITEM)
                .WithMessage(item => $"A quantidade máxima de um item é {CarrinhoCliente.MAX_QUANTIDADE_ITEM}");

            RuleFor(c => c.Valor)
                .GreaterThan(0)
                .WithMessage(item => "O valor do precisa ser maior que 0");
        }
    }
}
